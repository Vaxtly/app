<script lang="ts">
  ...
  let filteredEnvironments = $derived.by(() => {
    ...
    const list = q
      ? environmentsStore.environments.filter((e) => e.name.toLowerCase().includes(q))
      : environmentsStore.environments
    // Default env first, then selected, then the rest (stable within groups)
    const rank = (e: { id: string }): number =>
      e.id === state?.defaultEnvironmentId ? 0 : selectedEnvIds.has(e.id) ? 1 : 2
      ...
      <div class="ce-env-list">
        {#each filteredEnvironments as env (env.id)}
          ...
          <div
            ...
            onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleEnv(env.id) } }}
          >
            <span class="ce-env-check" class:ce-env-check--on={isSelected}>
              {#if isSelected}
                <svg class="ce-env-check-mark h-3 w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M5 13l4 4L19 7" /></svg>
              {/if}
            </span>

            <span class="min-w-0 flex-1 truncate text-left text-xs {isSelected ? 'font-medium text-surface-100' : 'text-surface-400'}">{env.name}</span>
...
<style>
  ...
  .ce-env-row--selected:hover {
    background: color-mix(in srgb, var(--color-brand-500) 11%, transparent);
  }
  ...
  .ce-env-check-mark {
    animation: ce-check-pop 0.16s ease-out;
  }

  @keyframes ce-check-pop {
  ...
  .ce-env-row:focus-visible .ce-env-star,
  .ce-env-row--selected .ce-env-star {
    opacity: 1;
  }
  ...
  .ce-env-row .ce-env-star--active,
  .ce-env-row .ce-env-star--active:hover {
